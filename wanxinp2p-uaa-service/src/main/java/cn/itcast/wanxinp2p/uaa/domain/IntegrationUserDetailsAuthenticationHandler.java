...
public class IntegrationUserDetailsAuthenticationHandler {
	...
	/**
	 * 认证处理
	 * 此方法返回该对象给Spring Security OAuth2框架，Spring Security OAuth2框架会根据里面的内容生成jwt令牌，从而使令牌中保存了登录用户的相关数据。
	 * @param domain 用户域 ，如b端用户、c端用户等，由接入方传入
	 * @param authenticationType  认证类型，如密码认证，短信认证等，由接入方传入
	 * @param token   SpringSecurity的token对象，里面存放了接入方调用UAA接口进行认证时传入的用户名、密码等需要验证的信息
	 * @return  UnifiedUserDetails 登录成功   SpringSecurity对象，用于存放登录成功时返回的信息，比如账号基本信息、权限、资源等。
	 */
